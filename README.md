...
| `src/finiteT_mc.py` | MC + divided differences for $\Phi = -\beta_{\min}^{-1}\log\int [Z_y]^k d\mathbf{x}$ |
| `src/divided_differences.py` | Exact simplex integral $\int \delta(m-\sum y_j)e^{\sum a_j y_j}dy$ via DD |
